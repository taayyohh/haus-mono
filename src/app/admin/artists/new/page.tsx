...
export default function NewArtistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch('/api/graphql', {
      ...
      credentials: 'include',
      body: JSON.stringify({
        query: `mutation { createArtist(input: {
          name: "${form.name}",
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl uppercase text-white mb-8">New Artist</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm text-white/60 mb-1">Name</label>
          <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full p-3 bg-[#1b1b1b] border border-white-13 text-white text-sm" required />
        </div>
        <div>
          <label className="block text-sm text-white/60 mb-1">Bio</label>
          <textarea value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} rows={8} className="w-full p-3 bg-[#1b1b1b] border border-white-13 text-white text-sm" required />
        </div>
        <div>
        ...
        <button type="submit" disabled={saving} className="w-full py-3 bg-white text-black text-sm uppercase font-medium hover:bg-gray-200 disabled:opacity-50">
